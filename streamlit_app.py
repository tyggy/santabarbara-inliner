...
def main():
    ...
    with st.form("premailer_form"):
        ...
        # New checkbox for removing specific elements
        remove_specific_elements = st.checkbox("Remove date tags and export buttons?", value=False)
    ...
    if submitted:
        ...
        # Remove specific elements if checkbox is checked
        if remove_specific_elements:
            # Remove div elements with class 'eventlist-datetag'
            date_tags = final_snippet.find_all("div", class_="eventlist-datetag")
            for tag in date_tags:
                tag.decompose()
            
            # Remove a elements with class 'eventlist-button'
            button_links = final_snippet.find_all("a", class_="eventlist-button")
            for link in button_links:
                link.decompose()
            
            # Remove a elements with class 'eventlist-meta-export-google'
            google_export_links = final_snippet.find_all("a", class_="eventlist-meta-export-google")
            for link in google_export_links:
                link.decompose()
            
            # Remove a elements with class 'eventlist-meta-export-ical'
            ical_export_links = final_snippet.find_all("a", class_="eventlist-meta-export-ical")
            for link in ical_export_links:
                link.decompose()
